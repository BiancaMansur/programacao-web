...
async function initDb() {
  if (!await db.schema.hasTable('perfis')) {
    await db.schema.createTable('perfis', t => {
      t.increments('id').primary();
      t.string('nome').unique().notNullable();
    });
    await db('perfis').insert([{ nome: 'admin' }, { nome: 'aluno' }]);
  }
  ...
  if (!await db.schema.hasTable('usuarios')) {
    await db.schema.createTable('usuarios', t => {
      ...
      t.integer('perfil_id').references('id').inTable('perfis').onDelete('SET NULL');
    });
  }
